package com.xinhu.wrokflow.listener;
...
import com.xinhu.wrokflow.domain.bo.FlowCopyBo;
import com.xinhu.wrokflow.domain.vo.NodeExtVo;
import com.xinhu.wrokflow.handler.FlowProcessEventHandler;
import com.xinhu.wrokflow.service.IFlwCommonService;
import com.xinhu.wrokflow.service.IFlwInstanceService;
...
public class WorkflowGlobalListener implements GlobalListener {
    ...
    public void assignment(ListenerVariable listenerVariable) {
        ...
        for (Task flowTask : nextTasks) {
            ...
            if (variable.containsKey(flowTask.getNodeCode()) && TaskStatusEnum.isPassOrBack(flowParams.getHisStatus())) {
                ...
                flowTask.setPermissionList(Arrays.asList(userIds.split(StringUtils.SEPARATOR)));
                variable.remove(flowTask.getNodeCode());
            }
            // 如果是申请节点，则把启动人添加到办理人
            if (flowTask.getNodeCode().equals(applyNodeCode)) {
                flowTask.setPermissionList(Arrays.asList(instance.getCreateBy()));
            }
        }
    }
    ...
    public void finish(ListenerVariable listenerVariable) {
        ...
        if (variable.containsKey(FlowConstant.FLOW_COPY_LIST)) {
            List<FlowCopyBo> flowCopyList = MapUtil.get(variable, FlowConstant.FLOW_COPY_LIST, new TypeReference<List<FlowCopyBo>>() {
            });
            // 添加抄送人
            flwTaskService.setCopy(task, flowCopyList);
        }
        if (variable.containsKey(FlowConstant.MESSAGE_TYPE)) {
            List<String> messageType = MapUtil.get(variable, FlowConstant.MESSAGE_TYPE, new TypeReference<List<String>>() {
            });
