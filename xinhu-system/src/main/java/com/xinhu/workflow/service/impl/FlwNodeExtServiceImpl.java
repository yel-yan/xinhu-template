...
import com.xinhu.common.core.domain.dto.DictTypeDTO;
...
import com.xinhu.workflow.common.enums.ButtonPermissionEnum;
import com.xinhu.workflow.common.enums.CopySettingEnum;
import com.xinhu.workflow.common.enums.NodeExtEnum;
import com.xinhu.workflow.common.enums.VariablesEnum;
import com.xinhu.workflow.domain.vo.ButtonPermissionVo;
...
public class FlwNodeExtServiceImpl implements NodeExtService, IFlwNodeExtService {
    ...
    static {
        CHILD_NODE_MAP = Collections.unmodifiableMap(new HashMap<String, Map<String, Object>>() {{
            put(CopySettingEnum.class.getSimpleName(), new HashMap<String, Object>() {{
                put("label", "抄送对象");
                put("type", 5);
                put("must", false);
                put("multiple", false);
                put("desc", "设置该节点的抄送办理人");
            }});
            put(VariablesEnum.class.getSimpleName(), new HashMap<String, Object>() {{
                put("label", "自定义参数");
                put("type", 2);
                put("must", false);
                put("multiple", false);
                put("desc", "节点执行时可设置自定义参数，多个参数以逗号分隔，如：key1=value1,key2=value2");
            }});
            put(ButtonPermissionEnum.class.getSimpleName(), new HashMap<String, Object>() {{
                put("label", "权限按钮");
                put("type", 4);
                put("must", false);
                put("multiple", true);
    ...
    public List<NodeExt> getNodeExt() {
        ...
        nodeExtList.add(buildNodeExt("wf_basic_tab", "基础设置", 1,
            Arrays.asList(CopySettingEnum.class, VariablesEnum.class)));
        // 构建按钮权限页面
        nodeExtList.add(buildNodeExt("wf_button_tab", "权限", 2,
            Arrays.asList(ButtonPermissionEnum.class)));
        ...
        nodeExt.setChilds(sources.stream()
            .map(source -> {
                if (source instanceof Class<?>) {
                    ...
                    if (NodeExtEnum.class.isAssignableFrom(clazz)) {
                        return buildChildNode((Class<? extends NodeExtEnum>) clazz);
                    }
                } else if (source instanceof String) {
                    String dictType = (String) source;
