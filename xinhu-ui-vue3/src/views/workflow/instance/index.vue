...
const handleSelectionChange = (selection) => {
  ...
  instanceIds.value = selection.map((item) => item.id);
...
const cancelPopover = async (index) => {
  (proxy?.$refs[`popoverRef${index}`]).hide(); //关闭弹窗
};
/** 查看按钮操作 */
...
//查询流程变量
const handleInstanceVariable = async (row) => {
 ...
 * @param data 原始数据
 */
function formatToJsonObject(data) {
  try {
...
const handleVariable = async (formEl) => {
